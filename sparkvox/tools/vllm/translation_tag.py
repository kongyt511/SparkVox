...
def build_one_prompt(meta, tokenizer):
    prompt_zh = "Please translate the following text into English (Note that, aside from the translation, no other responses or explanations should be provided.):"
    prompt_en = "请将以下文本翻译为中文(注意，除了翻译内容外，不要给出任何其他回答或解释):"
    text = meta["text"]
    if is_chinese(text):
        prompt = prompt_zh
    else:
    ...
    for i in tqdm(range(0, len(data), args.chunk_size), desc="Processing chunks"):
        ...
        with open(args.output, "a", encoding="utf-8") as f:
            for d, output in zip(chunk, outputs):
                d['qwen_trans'] = output.outputs[0].text
                # print(d)
